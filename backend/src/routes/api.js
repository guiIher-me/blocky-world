...
router.put("/worlds/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao atualizar as informações do mundo" });
  }
});
...
// Rota GET para pegar informações principais de documentos World
router.get("/worlds", async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: "Error ao obter mundos" });
  }
});

// Rota GET para pegar as informações de um documento World pelo id
